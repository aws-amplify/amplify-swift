...
final public class StorageCategory: Category {
    enum PluginOrSelector {
        case plugin(StorageCategoryPlugin)
        case selector(StoragePluginSelector)
    }
    ...
    var pluginOrSelector: PluginOrSelector {
        ...
        guard let selector = pluginSelectorFactory.makeSelector() as? StoragePluginSelector else {
            preconditionFailure(
                """
