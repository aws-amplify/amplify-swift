...
public struct HubFilters {
    ...
    /// Returns a HubFilter that is `true` if the event's `event` property matches the `event` argument
    /// - Parameter event: The event to match
    public static func hubFilter(forEvent event: String) -> HubFilter {
        let filter: HubFilter = { $0.event == event }
        return filter
    }
