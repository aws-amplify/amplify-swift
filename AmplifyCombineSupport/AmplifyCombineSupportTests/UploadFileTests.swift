...
class UploadFileTests: XCTestCase {
    ...
    func testOperationSucceeds() {
        ...
        plugin.responders.uploadFile = { _, _, _, _, resultListener in
            resultListener?(.success("Test"))
        }
        let sink = Amplify.Storage.uploadFile(key: "key", local: URL(fileURLWithPath: "file:///path/to/file"))
            .resultPublisher
    ...
    func testProgress() {
        ...
        let sink = Amplify.Storage.uploadFile(
            key: "key",
