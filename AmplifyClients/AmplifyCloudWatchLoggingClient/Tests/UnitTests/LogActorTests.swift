...
final class LogActorTests: XCTestCase {
    ...
    func testLogActorTriggersFileRotationOnRecord() async throws {
        ...
        let entries = (0 ..< numberOfEntries).map { LogEntry(namespace: "", level: .error, message: "\($0)", created: .init(timeIntervalSince1970: Double($0))) }
        ...
        XCTAssertEqual(rotations.map(\.lastPathComponent), [
            "amplify.0.log",
        ])
