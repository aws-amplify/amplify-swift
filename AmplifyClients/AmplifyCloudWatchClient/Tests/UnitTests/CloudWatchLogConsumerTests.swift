...
final class CloudWatchLogConsumerTests: XCTestCase {
    ...
    func testClientThrowsOnClientError() async throws {
        ...
        do {
            ...
            guard case let .service(_, _, underlyingError) = error else {
                return XCTFail("Expected CloudWatchError.service, got \(error)")
            }
            XCTAssertEqual(underlyingError.map { String(describing: $0) }, "consumeError")
        }
        XCTAssertEqual(client.interactions, [
            "describeLogStreams(input:)",
            "createLogStream(input:)",
