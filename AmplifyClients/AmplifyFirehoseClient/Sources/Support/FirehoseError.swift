...
/// Top-level error type for Firehose operations.
...
extension FirehoseError: AmplifyError {
    ...
    /// Maps a raw error into a FirehoseError, handling RecordCacheError and unknown errors.
    static func from(_ error: Error) -> FirehoseError {
        if let firehoseError = error as? FirehoseError {
            return firehoseError
        }
    ...
    private static func fromCacheError(_ cacheError: RecordCacheError) -> FirehoseError {
        switch cacheError {
