...
extension AWSLocationGeoPlugin {
    ...
    public func startTracking(for device: Geo.Device,
        ...
        guard options.tracker == nil, pluginConfig.defaultTracker != nil else {
            throw Geo.Error.invalidConfiguration(
                GeoPluginErrorConstants.missingTracker.errorDescription,
                GeoPluginErrorConstants.missingTracker.recoverySuggestion)
        }
        ...
        if options.tracker == nil {
            var optionsWithDefaultTracker: Geo.LocationManager.TrackingSessionOptions = Geo.LocationManager.TrackingSessionOptions(options: options)
            optionsWithDefaultTracker.tracker = pluginConfig.defaultTracker
            if Self.deviceTracker == nil {
                Self.deviceTracker = AWSDeviceTracker(locationManager: Geo.LocationManager(options: optionsWithDefaultTracker))
            }
            Self.deviceTracker?.configure(with: optionsWithDefaultTracker)
        } else {
            if Self.deviceTracker == nil {
                Self.deviceTracker = AWSDeviceTracker(locationManager: Geo.LocationManager(options: options))
            }
            Self.deviceTracker?.configure(with: options)
        }
