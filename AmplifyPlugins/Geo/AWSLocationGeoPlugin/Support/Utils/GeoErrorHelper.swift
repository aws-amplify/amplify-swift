...
class GeoErrorHelper {
    ...
    static func mapAWSLocationError(_ error: Error) -> Geo.Error {
        ...
        } else if let updateLocationError = error as? BatchUpdateDevicePositionOutputError {
            return GeoErrorHelper.mapError(error: updateLocationError) ?? defaultError
        } else if let deleteLocationHistoryError = error as? BatchDeleteDevicePositionHistoryOutputError {
            return GeoErrorHelper.mapError(error: deleteLocationHistoryError) ?? defaultError
        }
    ...
    static func mapError(error: BatchUpdateDevicePositionOutputError) -> Geo.Error? {
        switch error {
        case .accessDeniedException(let accessDeniedException):
            return Geo.Error.accessDenied(accessDeniedException.message ?? "", GeoPluginErrorConstants.accessDenied, error)
        case .internalServerException(let internalServerException):
    ...
    static func mapError(error: BatchDeleteDevicePositionHistoryOutputError) -> Geo.Error? {
        switch error {
        case .accessDeniedException(let accessDeniedException):
            return Geo.Error.accessDenied(accessDeniedException.message ?? "", GeoPluginErrorConstants.accessDenied, error)
        case .internalServerException(let internalServerException):
