...
struct TextractClient {
    ...
    private func request<Input, Output>(
        action: TextractAction<Input, Output>,
        input: Input,
        file: String = #fileID,
    ...
    func detectDocumentText(input: DetectDocumentTextInput) async throws -> DetectDocumentTextOutputResponse {
        try await request(
            action: .detectDocumentText(input: input),
            input: input
        )
    }
    ...
    func analyzeDocument(input: AnalyzeDocumentInput) async throws -> AnalyzeDocumentOutputResponse {
        try await request(
            action: .analyzeDocument(input: input),
            input: input
        )
    }
}

extension TextractClient: DefaultLogger {
    public static let log: Logger = {
        Amplify.Logging.logger(
            forCategory: "Predictions",
            forNamespace: "TextractClient"
        )
    }()
