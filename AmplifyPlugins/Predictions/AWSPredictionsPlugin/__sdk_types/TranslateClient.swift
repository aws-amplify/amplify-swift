...
struct TranslateClient {
    ...
    private func request<Input, Output>(
        action: TranslateAction<Input, Output>,
        input: Input,
        file: String = #fileID,
    ...
    func translateText(input: TranslateTextInput) async throws -> TranslateTextOutputResponse {
        try await request(action: .translateText(input: input), input: input)
    }
}


extension TranslateClient: DefaultLogger {
