...
final class GraphQLLazyLoadPostComment7Tests: GraphQLLazyLoadBaseTest {
    ...
    func testCommentWithLazyLoadPost() async throws {
        ...
        assertLazyReference(createdComment._post, state: .notLoaded(identifiers: [
            .init(name: "postId", value: createdPost.postId),
    ...
    func assertComment(
        _ comment: Comment,
        canLazyLoad post: Post
    ) async throws {
        assertLazyReference(comment._post, state: .notLoaded(identifiers: [
            .init(name: "postId", value: post.postId),
            .init(name: "title", value: post.title)
        ]))
        guard let loadedPost = try await comment.post else {
