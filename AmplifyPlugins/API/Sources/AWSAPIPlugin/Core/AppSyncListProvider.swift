...
public class AppSyncListProvider<Element: Model>: ModelListProvider {
    ...
    convenience init(metadata: AppSyncListDecoder.Metadata) {
        self.init(
            associatedIdentifiers: metadata.appSyncAssociatedIdentifiers,
    ...
    func load(
        ...
        do {
            ...
            case .success(let graphQLData):
                guard let listResponse = try? AppSyncListResponse.initWithMetadata(
                    type: Element.self,
    ...
    func getNextPage(
        nextToken: String,
        filter: [String: Any]?
    ) async throws -> List<Element> {
        let request = GraphQLRequest<List<Element>>.listQuery(
            responseType: List<Element>.self,
            modelSchema: Element.schema,
            filter: filter,
            limit: limit,
            nextToken: nextToken,
