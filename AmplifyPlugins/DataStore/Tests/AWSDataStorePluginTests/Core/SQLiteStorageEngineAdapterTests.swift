...
class SQLiteStorageEngineAdapterTests: BaseDataStoreTests {
    ...
    func testInsertPost() {
        ...
        storageAdapter.save(post, eagerLoad: true)
    ...
    func testInsertPostWithAll() {
        // insert a post
        let post = Post(title: "title", content: "content", createdAt: .now())
        storageAdapter.save(post, modelSchema: post.schema, condition: QueryPredicateConstant.all, eagerLoad: true)
            .map { $0.0 }
            .flatMap { savedPost in
                storageAdapter.query(Post.self, modelSchema: post.schema, condition: nil, sort: nil, paginationInput: nil, eagerLoad: true)
            }
            .ifSuccess { posts in
                XCTAssert(posts.count == 1)
                if let savedPost = posts.first {
    ...
    func testInsertPostAndThenUpdateIt() {
        ...
        storageAdapter.save(post, eagerLoad: true)
            .flatMap { _ in
                ...
                updatingPost.title = "title updated"
                return storageAdapter.save(updatingPost, eagerLoad: true)
            }
            .flatMap { _ in
                storageAdapter.query(
                    Post.self,
            ...
            .ifSuccess { posts in
                XCTAssertEqual(posts.count, 1)
                if let queriedPost = posts.first {
                    XCTAssertEqual(queriedPost.id, post.id)
                    XCTAssertEqual(queriedPost.title, "title updated")
                }
            }
            .ifFailure { error in
                XCTFail(String(describing: error))
