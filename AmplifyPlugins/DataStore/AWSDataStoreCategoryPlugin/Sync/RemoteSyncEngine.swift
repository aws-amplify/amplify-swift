...
class RemoteSyncEngine: RemoteSyncEngineBehavior {
    ...
    private func initializeSubscriptions(api: APICategoryGraphQLBehavior,
        ...
        log.debug(#function)
        let modelSchemas = ModelRegistry.modelSchemas.filter { $0.isSyncable }
        reconciliationQueue = reconciliationQueueFactory(modelSchemas, api, storageAdapter, auth, nil)
