...
final class AWSCloudWatchLoggingFilterTests: XCTestCase {
    ...
    func testDefaultsToRemoteConfigurationCanLog() {
        ...
        localStore.setLocalLoggingConstraints(loggingConstraints: remoteLoggingConstraints)
        let resolver = AWSCloudWatchLoggingConstraintsResolver(loggingPluginConfiguration: configuration, loggingConstraintsLocalStore: localStore)
        filter = AWSCloudWatchLoggingFilter(loggingConstraintsResolver: resolver)
