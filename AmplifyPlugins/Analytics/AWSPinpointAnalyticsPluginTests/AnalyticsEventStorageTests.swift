...
class AnalyticsEventStorageTests: XCTestCase {
  ...
  override func setUp() {
    do {
      ...
      let basicEvent: [Binding] = [
        1, encodedAttributes, "eventType", encodedMetrics,
        DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
          from: "2022-06-10T18:50:20.618+0000")!.utcTimeMillis, 1, "2022-06-10T17:00:20.618+0000",
        "2022-06-10T17:10:20.618+0000", 1_654_904_585, 0, 0
      ]
      let failedWithMaxRetry: [Binding] = [
        2, encodedAttributes, "eventType", encodedMetrics,
        DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
          from: "2022-06-9T18:50:20.618+0000")!.utcTimeMillis, 2, "2022-06-9T17:00:20.618+0000",
        "2022-06-9T17:10:20.618+0000", 1_654_818_185, 0, 4
      ]
      let dirtyEvent: [Binding] = [
        3, encodedAttributes, "eventType", encodedMetrics,
        DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
          from: "2022-06-8T18:50:20.618+0000")!.utcTimeMillis, 3, "2022-06-8T17:00:20.618+0000",
        "2022-06-8T17:10:20.618+0000", 1_654_731_785, 1, 3
      ]
      let dirtyEvent2: [Binding] = [
        4, encodedAttributes, "eventType", encodedMetrics,
        DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
          from: "2022-06-7T18:50:20.618+0000")!.utcTimeMillis, 4, "2022-06-7T17:00:20.618+0000",
        "2022-06-7T17:10:20.618+0000", 1_654_645_385, 1, 3
      ]
      let eventWithDirtyFlag: [Binding] = [
        5, encodedAttributes, "eventType", encodedMetrics,
        DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
          from: "2022-06-6T18:50:20.618+0000")!.utcTimeMillis, 5, "2022-06-6T17:00:20.618+0000",
        "2022-06-6T17:10:20.618+0000", 1_654_558_985, 1, 1
      ]
  ...
  func testDiskUsageCheckUnderLimit() {
    do {
      ...
      var dirtyEventcount =
        try adapter.executeQuery(dirtyEventCountStatement, []).scalar() as! Int64
  ...
  func testPinpointEventConversion() {
    do {
      let expectedSessionStartTime = DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
        from: "2022-06-10T17:00:20.618+0000")
      let expectedSessionStopTime = DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
        from: "2022-06-10T17:10:20.618+0000")
      let events = try storage.getEventsWith(limit: 5)
      let latestEvent = events.first(where: { $0.id == "1" })
      XCTAssertNotNil(latestEvent)
      XCTAssertEqual(latestEvent?.eventType, "eventType")
      XCTAssertEqual(latestEvent?.eventTimestamp, 1_654_887_020_618)
  ...
  func testSaveEvent() {
    do {
      let expectedSessionStartTime = DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
        from: "2022-06-11T17:00:20.618+0000")
      let expectedSessionStopTime = DateFormatter.iso8601DateFormatterWithFractionalSeconds.date(
        from: "2022-06-11T17:10:20.618+0000")
      ...
      let session = PinpointSession(
        sessionId: "6", startTime: expectedSessionStartTime!, stopTime: expectedSessionStopTime)
      let event = PinpointEvent(
        id: "6", eventType: "newEventType", eventTimestamp: expectedSessionStartTime!.utcTimeMillis,
        session: session)
