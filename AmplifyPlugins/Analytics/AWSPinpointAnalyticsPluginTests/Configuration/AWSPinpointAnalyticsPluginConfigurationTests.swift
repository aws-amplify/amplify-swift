...
class AWSPinpointAnalyticsPluginConfigurationTests: XCTestCase {
  ...
  let pinpointAnalyticsPluginConfiguration = JSONValue(
    dictionaryLiteral: (AWSPinpointAnalyticsPluginConfiguration.appIdConfigKey, "testAppId"),
    (AWSPinpointAnalyticsPluginConfiguration.regionConfigKey, "us-east-1")
  )
  let regionConfiguration = JSONValue(
    dictionaryLiteral: (AWSPinpointAnalyticsPluginConfiguration.regionConfigKey, "us-east-1"))

  func testConfigureSuccess() throws {
    let analyticsPluginConfig = JSONValue(
      dictionaryLiteral: (
        AWSPinpointAnalyticsPluginConfiguration.pinpointAnalyticsConfigKey,
  ...
  func testConfigureThrowsErrorForInvalidAutoFlushEventsIntervalValue() {
    ...
    XCTAssertThrowsError(try AWSPinpointAnalyticsPluginConfiguration(analyticsPluginConfig)) {
      error in
  ...
  func testConfigureWithAutoSessionTrackingIntervalSuccess() {
    let analyticsPluginConfig = JSONValue(
      ...
      (AWSPinpointAnalyticsPluginConfiguration.pinpointTargetingConfigKey, regionConfiguration),
      (
        AWSPinpointAnalyticsPluginConfiguration.autoSessionTrackingIntervalKey,
    ...
    do {
      ...
      XCTAssertEqual(
        ...
        AWSPinpointAnalyticsPluginConfiguration.defaultAutoFlushEventsInterval)
      XCTAssertEqual(
        config.trackAppSessions, AWSPinpointAnalyticsPluginConfiguration.defaultTrackAppSession)
  ...
  func testConfigureThrowsErrorForMissingPinpointAnalyticsAppId() {
    let pinpointAnalyticsPluginConfiguration = JSONValue(
      dictionaryLiteral: (AWSPinpointAnalyticsPluginConfiguration.regionConfigKey, region))
    let analyticsPluginConfig = JSONValue(
      dictionaryLiteral: (
        AWSPinpointAnalyticsPluginConfiguration.pinpointAnalyticsConfigKey,
        pinpointAnalyticsPluginConfiguration
  ...
  func testConfigureThrowsErrorForEmptyPinpointAnalyticsAppIdValue() {
    let pinpointAnalyticsPluginConfiguration = JSONValue(
      dictionaryLiteral: (AWSPinpointAnalyticsPluginConfiguration.appIdConfigKey, ""),
  ...
  func testConfigureThrowsErrorForInvalidPinpointAnalyticsAppIdValue() {
    let pinpointAnalyticsPluginConfiguration = JSONValue(
      dictionaryLiteral: (AWSPinpointAnalyticsPluginConfiguration.appIdConfigKey, 1),
      (AWSPinpointAnalyticsPluginConfiguration.regionConfigKey, region)
    )
    let analyticsPluginConfig = JSONValue(
      dictionaryLiteral: (
  ...
  func testConfigureThrowsErrorForMissingPinpointAnalyticsRegion() {
    let pinpointAnalyticsPluginConfiguration = JSONValue(
      dictionaryLiteral: (AWSPinpointAnalyticsPluginConfiguration.appIdConfigKey, appId))
    let analyticsPluginConfig = JSONValue(
      dictionaryLiteral: (
        AWSPinpointAnalyticsPluginConfiguration.pinpointAnalyticsConfigKey,
        pinpointAnalyticsPluginConfiguration
  ...
  func testConfigureThrowsErrorForMissingPinpointTargetingRegion() {
    let regionConfiguration = JSONValue(dictionaryLiteral: ("MissingRegionKey", region))
    let analyticsPluginConfig = JSONValue(
      dictionaryLiteral: (
        AWSPinpointAnalyticsPluginConfiguration.pinpointAnalyticsConfigKey,
        pinpointAnalyticsPluginConfiguration
  ...
  func testConfigureThrowsErrorForEmptyPinpointTargetingRegionValue() {
    let regionConfiguration = JSONValue(
      dictionaryLiteral: (AWSPinpointAnalyticsPluginConfiguration.regionConfigKey, ""))
    let analyticsPluginConfig = JSONValue(
      dictionaryLiteral: (
        AWSPinpointAnalyticsPluginConfiguration.pinpointAnalyticsConfigKey,
        pinpointAnalyticsPluginConfiguration
  ...
  func testThrowsOnMissingConfig() throws {
    ...
    do {
      ...
      XCTFail(
        "Should have thrown a pluginConfigurationError if not supplied with a plugin-specific config."
      )
    } catch {
