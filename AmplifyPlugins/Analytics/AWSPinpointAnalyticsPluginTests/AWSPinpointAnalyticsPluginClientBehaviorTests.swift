...
class AWSPinpointAnalyticsPluginClientBehaviorTests: AWSPinpointAnalyticsPluginTestBase {
  ...
  func testIdentifyUser() throws {
    ...
    let analyticsEventReceived = expectation(
      description: "Analytics event was received on the hub plugin")
  ...
  func testRecordEvent() {
    let expectedPinpointEvent = PinpointEvent(
      eventType: testName, session: PinpointSession(appId: "", uniqueId: ""))
  ...
  func testRecordEventDispatchesErrorForPinpointError() {
    mockPinpoint.recordResult = .failure(
      NSError(
        domain: "domain",
        code: 1,
        userInfo: nil))
  ...
  func testFlushEventsDispatchesErrorForPinpointError() {
    mockPinpoint.submitEventsResult = .failure(
      NSError(
        domain: "domain",
        code: 1,
        userInfo: nil))
