...
extension PinpointEvent {
  ...
  func getInsertBindings(
    dateFormater: DateFormatter = DateFormatter.iso8601DateFormatterWithFractionalSeconds,
    archiver: AmplifyArchiverBehaviour = PinpointEvent.archiver
  ) -> [Binding?] {
  ...
  static func convertToEvent(
    _ element: Statement.Element,
    dateFormater: DateFormatter = DateFormatter.iso8601DateFormatterWithFractionalSeconds,
    archiver: AmplifyArchiverBehaviour = PinpointEvent.archiver
  ) -> PinpointEvent? {
    guard let sessionId = element[EventPropertyIndex.sessionId] as? String,
      let startTime = element[EventPropertyIndex.sessionStartTime] as? String,
      let startDateTime = dateFormater.date(from: startTime)
    else {
      return nil
    }
    ...
    let session = PinpointSession(
      sessionId: sessionId, startTime: startDateTime, stopTime: stopDateTime)
    ...
    guard let eventType = element[EventPropertyIndex.eventType] as? String,
      ...
      let timestamp = Date.Millisecond(eventTimestampValue)
    else {
      return nil
    }
    ...
    let pinpointEvent = PinpointEvent(
      id: eventId, eventType: eventType, eventTimestamp: timestamp, session: session)

    if let attributes = element[EventPropertyIndex.attributes] as? String,
      let data = Data(base64Encoded: attributes),
      let decodedAttributes = try? archiver.decode(
        AnalyticsClient.PinpointEventAttributes.self, from: data) {
      for (key, value) in decodedAttributes {
        pinpointEvent.addAttribute(value, forKey: key)
      }
    }

    if let metrics = element[EventPropertyIndex.metrics] as? String,
      let data = Data(base64Encoded: metrics),
      let decodedMetrics = try? archiver.decode(
        AnalyticsClient.PinpointEventMetrics.self, from: data) {
