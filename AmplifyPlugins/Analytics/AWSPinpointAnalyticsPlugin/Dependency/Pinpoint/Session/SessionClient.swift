...
class SessionClient: SessionClientBehaviour {
  ...
  convenience init(
    ...
    self.init(
      activityTracker: activityTracker ?? ActivityTracker.create(from: context),
  ...
  init(
    activityTracker: ActivityTrackerBehaviour,
    ...
    session =
      Self.retrieveStoredSession(from: userDefaults, using: archiver) ?? PinpointSession.invalid
  }
  ...
  func validateOrRetrieveSession(_ session: PinpointSession?) -> PinpointSession {
    ...
    return PinpointSession(
      sessionId: PinpointSession.Constants.defaultSessionId,
      startTime: Date(),
      stopTime: Date())
  }
...
extension PinpointSession {
  fileprivate static var invalid = PinpointSession(
    sessionId: "InvalidId", startTime: Date(), stopTime: nil)
}
