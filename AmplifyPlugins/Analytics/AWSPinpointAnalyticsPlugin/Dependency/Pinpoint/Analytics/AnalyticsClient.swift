...
protocol AnalyticsClientBehaviour: Actor {
  ...
  nonisolated func createAppleMonetizationEvent(
    with transaction: SKPaymentTransaction,
    with product: SKProduct
  ) -> PinpointEvent
  nonisolated func createVirtualMonetizationEvent(
...
actor AnalyticsClient: AnalyticsClientBehaviour {
  ...
  convenience init(context: PinpointContext) {
    if let storage = try? AnalyticsEventSQLStorage(
      dbAdapter: SQLiteLocalStorageAdapter(
        prefixPath: Constants.eventRecorderStoragePathPrefix,
        databaseName: context.configuration.appId)),
      let eventRecorder = try? EventRecorder(
        appId: context.configuration.appId, storage: storage, pinpointClient: context.pinpointClient
      ) {
  ...
  nonisolated func createAppleMonetizationEvent(
    ...
    return createMonetizationEvent(
      withStore: Constants.PurchaseEvent.appleStore,
  ...
  nonisolated func createVirtualMonetizationEvent(
    ...
    return createMonetizationEvent(
      withStore: Constants.PurchaseEvent.virtual,
      productId: productId,
      quantity: quantity,
      itemPrice: itemPrice,
  ...
  private nonisolated func createMonetizationEvent(
    withStore store: String,
    productId: String,
    quantity: Int,
    itemPrice: Double,
    currencyCode: String?,
    ...
    let monetizationEvent = PinpointEvent(
      eventType: Constants.PurchaseEvent.name,
      session: context.sessionClient.currentSession)
    monetizationEvent.addAttribute(
      store,
      forKey: Constants.PurchaseEvent.Keys.store)
    monetizationEvent.addAttribute(
      productId,
      forKey: Constants.PurchaseEvent.Keys.productId)
    monetizationEvent.addMetric(
      quantity,
