...
public struct AWSPinpointAnalyticsPluginConfiguration {
  ...
  init(_ configuration: JSONValue) throws {
    ...
    guard
      let pinpointAnalyticsConfig =
        configObject[AWSPinpointAnalyticsPluginConfiguration.pinpointAnalyticsConfigKey]
    else {
      throw PluginError.pluginConfigurationError(
    ...
    guard
      let pinpointTargetingConfig = configObject[
    ...
    let region = try AWSPinpointAnalyticsPluginConfiguration.getRegion(
      pinpointAnalyticsConfigObject)
    let targetingRegion = try AWSPinpointAnalyticsPluginConfiguration.getRegion(
      pinpointTargetingConfigObject)
    let autoFlushEventsInterval =
      try AWSPinpointAnalyticsPluginConfiguration.getAutoFlushEventsInterval(configObject)
    let trackAppSessions = try AWSPinpointAnalyticsPluginConfiguration.getTrackAppSessions(
      configObject)
  ...
  private static func getAutoSessionTrackingInterval(_ configuration: [String: JSONValue]) throws
    -> Int {
