...
public final class AmplifyBigInt {
    ...
    public init(_ data: [UInt8]) {
        let error = mp_from_sbin(&value, data, data.count)
    ...
    public init(unsignedData data: [UInt8]) {
        let error = mp_from_ubin(&value, data, data.count)
        guard error == MP_OKAY else {
            fatalError("Could not create a signed number from data - \(error)")
        }
    }
    ...
    public func asString(radix: Int = 10) -> String {
        ...
        var written = size_t()
        let conversionResult = mp_to_radix(&value, &cString, stringLengthInt, &written, Int32(radix))
