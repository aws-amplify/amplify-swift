...
extension AWSCognitoAuthPlugin: AuthCategoryDeviceBehavior {
    ...
    public func fetchDevices( options: AuthFetchDevicesRequest.Options? = nil) async throws -> [AuthDevice] {
        ...
        let task = AWSAuthFetchDevicesTask(request, authStateMachine: authStateMachine, userPoolFactory: authEnvironment.cognitoUserPoolFactory)
    ...
    public func forgetDevice( _ device: AuthDevice? = nil, options: AuthForgetDeviceRequest.Options? = nil) async throws {
        ...
        let task = AWSAuthForgetDeviceTask(request, authStateMachine: authStateMachine, userPoolFactory: authEnvironment.cognitoUserPoolFactory)
        return try await task.value
        }
    ...
    public func rememberDevice( options: AuthRememberDeviceRequest.Options? = nil) async throws {
        ...
        let task = AWSAuthRememberDeviceTask(request, authStateMachine: authStateMachine, userPoolFactory: authEnvironment.cognitoUserPoolFactory)
        return try await task.value
    }
}
