...
class AWSAuthSignInWithOTPTask: AuthSignInWithOTPTask, DefaultLogger {
    ...
    init(_ request: AuthSignInWithOTPRequest,
        ...
        self.signInMetadataRequestMetadata = .init(
            signInMethod: .otp, action: .request, deliveryMedium: request.destination)
    }
    ...
    func execute() async throws -> AuthSignInResult {
        ...
        do {
            // Start OTP sign in
            return try await startOTPSignIn()
        } catch {

            log.error(error: error)
            // If OTP Sign in failed, send sign in cancellation event
    ...
    private func startOTPSignIn() async throws -> AuthSignInResult {
        if request.flow == .signUpAndSignIn {
            log.verbose("Starting OTP Passwordless Sign Up flow")
            // [HS] TODO: Proceed to sign up flow first
        }

        log.verbose("Starting OTP Passwordless Sign In flow")
    ...
    func parseAndValidate(signInResult: AuthSignInResult) async throws -> AuthSignInResult? {
        ...
        guard  let nextStepString = challengeParams?["nextStep"] else {
            ...
            throw AuthError.service(
                "Did not receive a valid next step for Passwordless OTP flow.",
                AmplifyErrorMessages.shouldNotHappenReportBugToAWS(), nil)
        }
