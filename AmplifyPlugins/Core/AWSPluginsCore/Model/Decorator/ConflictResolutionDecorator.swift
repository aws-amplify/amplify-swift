...
public struct ConflictResolutionDecorator: ModelBasedGraphQLDocumentDecorator {
    ...
    private func addConflictResolution(selectionSet: SelectionSet,
        ...
        if !primaryKeysOnly || graphQLType == .mutation {
            ...
            // (specifically for syncQuery). Selection set for mutation should not be reduced because it needs to be
            // the full selection set to send mutation events to older iOS clients, which do not have the reduced
            // subscription selection set. subscriptions and sync query is to receive data, so it can be reduced to
            // allow decoding to the LazyReference type.
