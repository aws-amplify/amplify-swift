...
public struct BedrockRuntimeAuthSchemeResolverParameters: SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
    public let operation: Swift.String
    // Region is used for SigV4 auth scheme
    public let region: Swift.String?
}

public protocol BedrockRuntimeAuthSchemeResolver: SmithyHTTPAuthAPI.AuthSchemeResolver {
    // Intentionally empty.
    // This is the parent protocol that all auth scheme resolver implementations of
    // the service BedrockRuntime must conform to.
}

public struct DefaultBedrockRuntimeAuthSchemeResolver: BedrockRuntimeAuthSchemeResolver {
    ...
    public func resolveAuthScheme(params: SmithyHTTPAuthAPI.AuthSchemeResolverParameters) throws -> [SmithyHTTPAuthAPI.AuthOption] {
        ...
        guard let serviceParams = params as? BedrockRuntimeAuthSchemeResolverParameters else {
    ...
    public func constructParameters(context: Smithy.Context) throws -> SmithyHTTPAuthAPI.AuthSchemeResolverParameters {
        ...
        return BedrockRuntimeAuthSchemeResolverParameters(operation: opName, region: opRegion)
    }
}
