...
extension WorkMailClient {
    /// Paginate over `[ListPersonalAccessTokensOutput]` results.
    ///
    /// When this operation is called, an `AsyncSequence` is created. AsyncSequences are lazy so no service
    /// calls are made until the sequence is iterated over. This also means there is no guarantee that the request is valid
    /// until then. If there are errors in your request, you will see the failures only after you start iterating.
    /// - Parameters:
    ///     - input: A `[ListPersonalAccessTokensInput]` to start pagination
    /// - Returns: An `AsyncSequence` that can iterate over `ListPersonalAccessTokensOutput`
    public func listPersonalAccessTokensPaginated(input: ListPersonalAccessTokensInput) -> ClientRuntime.PaginatorSequence<ListPersonalAccessTokensInput, ListPersonalAccessTokensOutput> {
        return ClientRuntime.PaginatorSequence<ListPersonalAccessTokensInput, ListPersonalAccessTokensOutput>(input: input, inputKey: \.nextToken, outputKey: \.nextToken, paginationFunction: self.listPersonalAccessTokens(input:))
    }
}

extension ListPersonalAccessTokensInput: ClientRuntime.PaginateToken {
    public func usingPaginationToken(_ token: Swift.String) -> ListPersonalAccessTokensInput {
        return ListPersonalAccessTokensInput(
            maxResults: self.maxResults,
            nextToken: token,
            organizationId: self.organizationId,
            userId: self.userId
...
extension PaginatorSequence where OperationStackInput == ListPersonalAccessTokensInput, OperationStackOutput == ListPersonalAccessTokensOutput {
    /// This paginator transforms the `AsyncSequence` returned by `listPersonalAccessTokensPaginated`
    /// to access the nested member `[WorkMailClientTypes.PersonalAccessTokenSummary]`
    /// - Returns: `[WorkMailClientTypes.PersonalAccessTokenSummary]`
    public func personalAccessTokenSummaries() async throws -> [WorkMailClientTypes.PersonalAccessTokenSummary] {
        return try await self.asyncCompactMap { item in item.personalAccessTokenSummaries }
    }
}
extension WorkMailClient {
