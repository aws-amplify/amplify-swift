...
class ModelCodableTests: XCTestCase {
    ...
    func testToJSON() {
        let createdAt = Date(timeIntervalSince1970: 1_000_000.123)
    ...
    func testDecodeWithFractionalSeconds() {
        ...
        XCTAssertEqual(post?.createdAt, Date(timeIntervalSince1970: 1_000_000.123))
    }
    ...
    func testDecodeWithoutFractionalSeconds() {
        ...
        XCTAssertEqual(post?.title, "title")
        XCTAssertEqual(post?.content, "content")
        XCTAssertEqual(post?.createdAt, Date(timeIntervalSince1970: 1_000_000))
    }
}
